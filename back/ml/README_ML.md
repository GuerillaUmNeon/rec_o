...
Each subpackage uses `train_local` locally. There is no upload script in this tree.
...
| `ARTIST_MODEL_BLOB_NAME` | API / deployment config | `models/knn_model_test_joris_slim.pkl` |
| `ARTIST_MODEL_LOCAL_FILENAME` | `train_local` saves (`models/`) | `knn_model_test_joris_slim.pkl` |
| `RELEASE_GROUP_MODEL_BLOB_NAME` | API / deployment config | `models/release_group_knn_model.pkl` |
...
Produced only after training finishes.
